...
export default function AccountSettingsScreen() {
  const { userTeacher, isDebug } = useTeacherLoginContext();
  ...
  const initials = userTeacher.name
    .split(" ")
    .map((w) => w[0])
    .join("")
    .slice(0, 2);
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: bg }]} contentContainerStyle={styles.content}>
      ...
      <Text style={[styles.name, { color: textColor }]}>{userTeacher.name}</Text>
      <Text style={[styles.role, { color: subtextColor }]}>Teacher Account</Text>

      <View style={[styles.infoCard, { backgroundColor: cardBg }]}>
        <InfoRow label="Name" value={userTeacher.name} textColor={textColor} subtextColor={subtextColor} />
        <InfoRow label="Role" value="Teacher" textColor={textColor} subtextColor={subtextColor} />
        <InfoRow label="Account Type" value={isDebug ? "Debug" : "AWS Cognito"} textColor={textColor} subtextColor={subtextColor} />
        <InfoRow label="User ID" value={userTeacher.userId} textColor={textColor} subtextColor={subtextColor} last />
      </View>
    </ScrollView>
  );
}
